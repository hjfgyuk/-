...
async def get_evaluations(detail_url):
    async with semaphore:
        ...
        browser = await launch(headless=True, devtools=False, args=[f'--window-size={1366},{768}', ],
                               userDataDir=r'D:\python1\bug\基础知识\userdata_taobao_search', )
        ...
        name = re.findall(r"<title>(.*?)</title>", await page.content(), re.S) if re.findall(r"<title>(.*?)</title>",
                                                                                             await page.content(),
                                                                                             re.S) else None
        store = re.findall(r'class="ShopHeader--title--2qsBE1A".*?title="(.*?)"', await page.content(),
                           re.S) if re.findall(
...
# async def write_evaluation(url, gd):
...
async def main():
    ...
    print(urls)
    data = []
    tasks = [get_evaluations(url) for url in urls]
    await asyncio.gather(*tasks)
    await    session.close()
    for task in tasks:
        result = await task
        data.append(result)
    with open('D:\python1\bug\bug_exercise\goods.txt', 'a') as file:
        for item in data:
            file.write(str(item) + '\n')
